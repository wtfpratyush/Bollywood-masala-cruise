...
import { Mail, ChevronDown, CheckCircle2, Loader2, Sparkles } from "lucide-react";
...
const QuickQuoteForm = ({ className = "" }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send form data via standard submission endpoint or webhook
      ...
      console.log("Form submitted locally:", err);
    } finally {
    ...
    return (
      <div className={`w-full bg-white rounded-3xl shadow-2xl shadow-indigo-900/10 p-6 sm:p-8 border border-gray-100 text-center ${className}`}>
        ...
        <p className="text-[14px] sm:text-[15px] text-gray-600 leading-relaxed max-w-md mx-auto mb-6">
          Your quote request has been received. Our dedicated cruise specialists will get back to you with custom pricing and stateroom availability in less than 24 hours.
        </p>

        <div className="bg-[#f8f9ff] border border-indigo-100 rounded-2xl p-4 text-left mb-6 max-w-sm mx-auto">
          <p className="text-[12px] font-bold text-[#4b3df5] uppercase tracking-wider mb-2">Request Summary</p>
          <div className="space-y-1 text-[13px] text-gray-700">
            {formData.trip && <p><span className="font-semibold text-gray-900">Trip:</span> {formData.trip}</p>}
